    ...
    try:
        ...
        if upin==1234:
            for choose in range(100):
                ...
                # Check dues Balance and pay balance
                elif cc=='3':
                    cur=mydb.cursor()
                    ...
                    for cos_id in fetch:
                        if cos_id[0]==cid:
                            ...
                            if x==1:
                                ...
                                mycur.execute(sql3)
                                name=mycur.fetchone()
                                mode=int(input('Choose payment mode: Cash/Debit card/Paytm/Online Payment'))
                                y1=datetime.datetime.now()
                                year=y1.strftime("%Y")
                                month=y1.strftime("%m")
                                day=y1.strftime("%d")
                                date=f"{year}-{month}-{day}"
                                time=y1.strftime("%X")
                                sql4=f"insert into pay values ({cos_id[0]},'{name[0]}','{mode}',{pay},{ubal},'{date}','{time}')"
                                mycur.execute(sql4)
                                mydb.commit()
                ...
                # Check costomer name
                    ...
                    x=mycur.fetchall()
                    for i in x:
                        ff=f"| Customer Id: {i[0]} | Customer Name: {i[1]} | Mobile_no: {i[2]} | Address: {i[3]} | Date: {i[4]} |"
                        for f1 in ff:
                            print('-',end='')
                        print()
                        print(ff)
